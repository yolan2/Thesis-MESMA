...
# Testing mode: subset data and skip inference for faster testing
TESTING_MODE <- FALSE
...
if (!exists("add_ppi_columns")) add_ppi_columns <- function(df, lat) {
  ...
  # Peak stats per location-year (March 1-based phenological year) (avoids per-DOY PPI time series)
  peak_df <- df %>%
  ...
  df <- df %>%
    left_join(peak_df %>% select(location_id, pheno_year, DVI_max, zenith.angle, PPI),
...
# Phenological year helper: returns calendar year to use for phenological analysis; phenological
# year starts on March 1, months Jan-Feb are considered part of the previous year's phenology.
...
stage2_geometric_unmix <- function(y, veg_libraries, topK = 2, feature_weights = NULL, comp_templates = NULL, compressed_stage1_lib = NULL) {
  veg_types <- names(veg_libraries)
  n_veg <- length(veg_types)
  
  if (n_veg == 0) {
    if (exists('TESTING_MODE') && isTRUE(TESTING_MODE)) cat("[DEBUG stage2_geometric_unmix] No veg types provided -> returning NULL\n")
    return(NULL)
  }
  ...
  if (length(top_candidates) == 0) {
    if (exists('TESTING_MODE') && isTRUE(TESTING_MODE)) cat(sprintf("[DEBUG stage2_geometric_unmix] top_candidates empty for veg_types=%s\n", paste(veg_types, collapse=",")))
    return(NULL)
  }
  ...
  if (length(top_candidates) == 1) {
    ...
    if (exists('TESTING_MODE') && isTRUE(TESTING_MODE)) cat(sprintf("[DEBUG stage2_geometric_unmix] Single veg %s: returning 100%% fraction\n", veg))
    return(list(
...
dbg_return_null <- function(reason = NULL) {
  if (exists("TESTING_MODE") && isTRUE(TESTING_MODE)) {
    # Try to infer location/year from parent frame if available for better diagnostics
...
build_barren_veg_library <- function(df_local, avail_idx, min_samples = 5) {
  ...
  # REQUIRE pure vegetation samples (no soil ≈ 1) - no fallback allowed
  veg_rows <- if ("no soil" %in% names(df_local)) {
    ...
    # Check if we have enough pure vegetation samples
    ...
    # No 'no soil' column - this is an error since we require pure vegetation samples
    stop("[Stage1] CRITICAL ERROR: 'no soil' column is required to identify pure vegetation samples (no soil ≈ 1). Cannot proceed with MESMA analysis.")
  }
...
build_stage1_lib <- function(STAGE1_LIB, grid_name = "full") {
  ...
  # Also build full version with ALL available indices (for Stage 2 soil subtraction)
  ...
  # Do not normalize Stage 1 endmembers here; return raw full-resolution matrices.
  # Keep raw matrices and the L2 norms for debugging/diagnostics only.
  }
  
  cat(sprintf("[build_stage1_lib] Stage 1 indices: %s\n", paste(avail_idx, collapse = ", ")))
  
  # Return indices used so observation vectors can be built with same indices
...
if (exists('DEBUG_STAGE1_VERBOSE', envir = globalenv()) && isTRUE(get('DEBUG_STAGE1_VERBOSE', envir = globalenv())) && exists('COMPRESSED_STAGE1_LIB') && !is.null(COMPRESSED_STAGE1_LIB)) {
  ...
  # Sample some vegetated location-years (if df_train exists)
  if (exists('df_train') && nrow(df_train) > 0) {
    ...
    if (nrow(sample_rows) > 0) {
      for (i in seq_len(nrow(sample_rows))) {
        ...
        dly_year <- df_train[df_train$location_id == row$location_id & df_train$year == row$year, , drop = FALSE]
        res_compress <- compress_trace(dly_year, COMPRESSED_STAGE1_LIB$indices, budget = TEMPORAL_BUDGET)
        if (!is.null(res_compress)) {
          y_vec <- res_compress$y
          res_stage1 <- geometric_stage1_unmix(y_vec, COMPRESSED_STAGE1_LIB$barren, COMPRESSED_STAGE1_LIB$vegetation)
          cat(sprintf("Sample %d: loc=%s year=%d t=%.6f veg_frac=%.6f resid=%.6f y_norm_raw=%.4f barren_norm_raw=%.4f veg_norm_raw=%.4f\n",
                      i, as.character(row$location_id), as.integer(row$year), res_stage1$projection[1], res_stage1$veg_frac, res_stage1$residual, sqrt(sum(y_vec^2)), COMPRESSED_STAGE1_LIB$norm_b, COMPRESSED_STAGE1_LIB$norm_v))
        }
      }
    }
  }
...
# Debugging: Verbose Stage 1 diagnostics (do not enable in production)
...
unmix_stage2_compressed <- function(veg_kept, veg_frac, y, grid_type, compressed_templates_accessor, mesma_lib, topK = TOPK_VARIANTS, feature_weights = NULL, optimized_library = NULL) {
  ...
  y_vec <- as.numeric(y)
    ...
    for (v in veg_types) {
      ...
      if (isTRUE(TESTING_MODE)) {
         cat(sprintf("[DEBUG Stage2] M_norm dim: %s, y_weighted_norm len: %d\n", paste(dim(lib_v$M_norm), collapse="x"), length(y_weighted_norm)))
      }
      sims <- tryCatch({
        as.numeric(lib_v$M_norm %*% y_weighted_norm)
      }, error = function(e) {
  ...
  y <- as.numeric(y)
  if (verbose_this_call || isTRUE(TESTING_MODE)) {
    cat(sprintf("[DEBUG Stage2] y length=%d, finite=%d, nan=%d\n", length(y), sum(is.finite(y)), sum(is.nan(y))))
  }
  ...
  if (is.null(geom_result)) {
    if (isTRUE(TESTING_MODE)) cat("[DEBUG Stage2] stage2_geometric_unmix returned NULL\n")
    return(NULL)
  }
  ...
  if (isTRUE(ENABLE_UNCERTAINTY) && !is.null(best$w) && !is.null(best$chosen)) {
    # Ensure comp_templates is available for bootstrap (needed if Fast Path was used)
    if (is.null(comp_templates)) {
        comp_templates <- list()
        for (v in veg_types) {
...
geometric_block_bootstrap <- function(y_vec, comp_templates, top_variants, chosen_ids, w_hat, 
  ...
  for (b in seq_len(B)) {
    ...
    if (variant_switch) {
      ...
      result <- stage2_geometric_unmix(y_boot, veg_libraries, topK = 2, comp_templates = NULL, compressed_stage1_lib = NULL)
    } else {
...
build_mesma_variants <- function(reduced_data, raw_lib_templates, min_cluster_size = 10) {
  ...
  # ----------------------------------------------------------------------------
  # Debug counter for stage 1 unmixing (using closure)
  ...
  geometric_stage1_unmix <- function(y, barren_endmember, vegetation_endmember) {
      ...
      # Debug: Print dimension info for first 3 calls
      stage1_debug_env$count <- stage1_debug_env$count + 1
      if (stage1_debug_env$count <= 3) {
        cat(sprintf("[DEBUG Stage1 #%d] Dimensions: y=%d, barren=%d, veg=%d\n", 
                    stage1_debug_env$count, length(X), length(B), length(V)))
        cat(sprintf("[DEBUG Stage1 #%d] Norms: y=%.4f, barren=%.4f, veg=%.4f\n",
                    stage1_debug_env$count, sqrt(sum(X^2)), sqrt(sum(B^2)), sqrt(sum(V^2))))
        cat(sprintf("[DEBUG Stage1 #%d] Means: y=%.4f, barren=%.4f, veg=%.4f\n",
                    stage1_debug_env$count, mean(X), mean(B), mean(V)))
      }
      
      len <- min(length(X), length(B), length(V)); X <- X[1:len]; B <- B[1:len]; V <- V[1:len]
      # Do not normalize shapes here; perform unmixing on raw signals (amplitude + shape)
      res <- geometric_project_and_unmix(X, B, V)
      
      # Debug: Print unmixing result for first 3 calls
      if (stage1_debug_env$count <= 3) {
        cat(sprintf("[DEBUG Stage1 #%d] Unmix result: t=%.4f, f1(barren)=%.4f, f2(veg)=%.4f, residual=%.4f\n",
                    stage1_debug_env$count, res$t, res$f1, res$f2, res$residual))
      }
...
# Post-build Stage 1 runtime debug - run only if enabled and COMPRESSED_STAGE1_LIB exists
if (exists('DEBUG_STAGE1_VERBOSE', envir = globalenv()) && isTRUE(get('DEBUG_STAGE1_VERBOSE', envir = globalenv())) && exists('COMPRESSED_STAGE1_LIB') && !is.null(COMPRESSED_STAGE1_LIB)) {
  cat('\n=== STAGE1 RUNTIME DEBUG BLOCK ===\n')
  sample_rows <- tryCatch({ df_train %>% dplyr::filter(.data$Veg %in% ALLOWED_VEG) %>% dplyr::distinct(location_id, year) %>% head(10) }, error = function(e) NULL)
  if (!is.null(sample_rows) && nrow(sample_rows) > 0) {
    for (i in seq_len(nrow(sample_rows))) {
      ...
      if (!is.null(raw_mat)) {
        ...
        cat(sprintf("sample %d: loc=%s year=%d y_norm_raw=%.4f t=%.6f f2=%.6f resid=%.6f barren_norm_raw=%.4f veg_norm_raw=%.4f\n", i, as.character(loc), as.integer(yr), sqrt(sum(y_vec^2)), ifelse(is.null(stage1_res$projection), NA_real_, stage1_res$projection[1]), stage1_res$veg_frac, stage1_res$residual, COMPRESSED_STAGE1_LIB$norm_b, COMPRESSED_STAGE1_LIB$norm_v))
      }
    }
  }
...
geometric_hierarchical_unmix_compressed <- function(y, compressed_stage1_lib, mesma_lib, grid_type = 'medium', topK = TOPK_VARIANTS) {
  ...
  # NOTE: Soil subtraction is now performed DURING unmixing (in fit_one_task),

  # AFTER Stage 1 estimates the barren fraction. This ensures:
  # 1. LDA/PCA learns from the full spectral signatures (including soil effects)
  # 2. Stage 2 vegetation differentiation operates on soil-corrected residuals
  # The correction is: Y_veg = Y_original - (f_barren * barren_prototype)
...
if (isTRUE(TESTING_MODE)) {
  # Testing mode: avoid loading large inference files to speed up local testing
  ...
  # In testing mode, limit to a small subset of locations for faster testing
  if (isTRUE(TESTING_MODE)) {
    # Prioritize locations with sufficient data to avoid sparse data artifacts
    obs_counts <- df_tasks %>%
    ...
    test_locations <- unique_locs[1:n_test_locs]
    
    test_loc_years <- test_loc_years[test_loc_years$location_id %in% test_locations, ]
    cat(sprintf("[TESTING MODE] Limited to %d locations (with most observations) for testing: %s\n",
                n_test_locs, paste(test_locations, collapse = ", ")))
  }
  ...
  task_list <- lapply(seq_len(n_loc_years), function(i) {
    list(loc = test_loc_years$location_id[i], yr = test_loc_years$pheno_year[i])
  })
...
compress_temporal_matrix <- function(data, temporal_budget, avail) {
    ...
    res_safe <- tryCatch(
      {
        ...
        if (length(unique(dly_year$doy)) < min_doy_needed) {
          ...
          if (min_doy_needed > MIN_UNIQUE_DOY_INFERENCE) {
            if (isTRUE(TESTING_MODE)) cat(sprintf("[DEBUG fit_one_task] Skipping: insufficient DOYs loc=%s yr=%d unique_doys=%d min_needed=%d\n", loc, yr, length(unique(dly_year$doy)), min_doy_needed))
            return(NULL)
          } else {
        ...
        # CRITICAL: Use SAME indices as COMPRESSED_STAGE1_LIB to ensure alignment
        stage1_indices <- if (!is.null(COMPRESSED_STAGE1_LIB) && !is.null(COMPRESSED_STAGE1_LIB$indices)) {
          COMPRESSED_STAGE1_LIB$indices
        } else {
          avail
        }
        ...
        if (isTRUE(TESTING_MODE) && exists('stage1_res')) {
          cat(sprintf("[DEBUG fit_one_task] loc=%s yr=%d stage1_veg_frac=%.6f stage1_residual=%.4f mean_proj=%.6f\n", loc, yr, as.numeric(veg_fraction_total), as.numeric(stage1_res$residual), ifelse(!is.null(stage1_res$projection), mean(stage1_res$projection, na.rm = TRUE), NA_real_)))
        }
        ...
        if (!is.finite(veg_fraction_total)) {
          if (exists("TESTING_MODE") && isTRUE(TESTING_MODE)) cat(sprintf("[DEBUG fit_one_task] Non-finite veg_fraction_total for %s/%d\n", loc, yr))
          return(NULL)
        }
        ...
        if (isTRUE(veg_fraction_total <= 0)) {
          ...
          if (isTRUE(TESTING_MODE)) {
            cat(sprintf("[DEBUG fit_one_task RETURN] loc=%s yr=%d veg_fraction=%.6f barren_fraction=%.6f stage1_residual=%.4f\n", loc, yr, veg_fraction_total, barren_fraction_total, ifelse(exists('stage1_res'), stage1_res$residual, NA_real_)))
            cat("[DEBUG fit_one_task RETURN] coef_df (first 3 rows):\n")
            print(utils::head(coef_df, 3))
            cat("[DEBUG fit_one_task RETURN] diag_df:\n")
            print(diag_df)
          }
        ...
        if (length(veg_kept) == 0) {
          if (isTRUE(TESTING_MODE)) {
            cat(sprintf("[DEBUG fit_one_task] No vegetation classes kept: loc=%s yr=%d names(lib)=%s\n", loc, yr, paste(names(lib), collapse = ",")))
          }
          return(dbg_return_null("no_veg_kept"))
        }

        # --- SOIL SUBTRACTION: Remove barren component before Stage 2 ---
        # y_veg = y_original - (barren_fraction * barren_prototype)
        # This gives Stage 2 the "pure vegetation" signal to differentiate
        
        # Rebuild y with FULL indices for Stage 2 if available (to match optimized library)
        y_stage2_raw <- y
        barren_prototype <- if (!is.null(COMPRESSED_STAGE1_LIB)) COMPRESSED_STAGE1_LIB$barren else NULL
        ...
        if (!is.null(COMPRESSED_STAGE1_LIB) && !is.null(COMPRESSED_STAGE1_LIB$indices_full)) {
           ...
           if (!is.null(raw_mat_stage2)) {
             ...
             if (isTRUE(TESTING_MODE)) cat(sprintf("[DEBUG fit_one_task] Rebuilt y for Stage 2 with %d indices (len=%d)\n", length(stage2_indices), length(y_stage2_raw)))
           }
        }
        ...
        if (isTRUE(TESTING_MODE)) {
          # Debug: show veg_kept, available variants and compressed template sizes
          cat(sprintf("[DEBUG fit_one_task] Before Stage2: veg_kept=%s\n", paste(veg_kept, collapse=",")))
          for (v in veg_kept) {
            nv <- if (!is.null(mesma_lib[[v]])) length(mesma_lib[[v]]) else 0
            nc <- if (!is.null(compressed_templates_accessor[[v]])) length(names(compressed_templates_accessor[[v]])) else 0
            cat(sprintf("[DEBUG fit_one_task] veg=%s: variants_mesa=%d, comp_templates=%d\n", v, nv, nc))
          }
          cat(sprintf("[DEBUG fit_one_task] y_for_stage2: length=%d finite=%d anyNaN=%s\n", length(y_for_stage2), sum(is.finite(y_for_stage2)), any(is.nan(y_for_stage2))))
        }
        mesma_result <- unmix_stage2_compressed(
          veg_kept, veg_fraction_total, y_for_stage2, "full", 
        ...
        if (is.null(mesma_result)) {
          if (isTRUE(TESTING_MODE)) cat(sprintf("[DEBUG fit_one_task] mesma_result NULL for loc=%s yr=%d veg_frac=%.6f veg_kept=%s\n", loc, yr, veg_fraction_total, paste(veg_kept, collapse = ",")))
        ...
        if (isTRUE(TESTING_MODE)) {
          cat(sprintf("[DEBUG fit_one_task RETURN] loc=%s yr=%d veg_fraction=%.6f barren_fraction=%.6f stage1_residual=%.4f stage2_rmse=%.4f\n", loc, yr, veg_fraction_total, barren_fraction_total, ifelse(exists('stage1_res'), stage1_res$residual, NA_real_), ifelse(!is.null(mesma_result), mesma_result$rmse, NA_real_)))
          cat("[DEBUG fit_one_task RETURN] top coef rows:\n")
          print(utils::head(coef_df, 5))
          cat("[DEBUG fit_one_task RETURN] diag_df:\n")
          print(diag_df)
          if (nrow(coef_df) > 0) {
            cat(sprintf("[DEBUG fit_one_task RETURN] max veg coef (non-barren) = %.6f\n", max(coef_df$coef[coef_df$Veg != 'barren'], na.rm = TRUE)))
          }
        }
        return(list(
          coef_df = coef_df,
  ...
  if (!"task_key" %in% names(df_tasks)) {
    df_tasks$task_key <- paste(df_tasks$location_id, df_tasks$pheno_year, sep = "_")
  }
  
  # Identify target tasks
  # Use pheno_year which is what 'task_key' uses (created from location_id + pheno_year)
  target_keys <- paste(test_loc_years$location_id, test_loc_years$pheno_year, sep = "_")
  ...
  for (i in seq_along(key_batches)) {
    ...
    results_list[names(batch_results)] <- batch_results
      ...
      for (k in names(batch_results)) {
        r <- batch_results[[k]]
        if (is.null(r)) {
          cat(sprintf("[DEBUG batch_result] training task %s returned NULL\n", k))
              # Re-run the task serially with additional debug prints to capture why it failed
              if (k %in% names(batch_task_list)) {
                if (isTRUE(TESTING_MODE)) cat(sprintf("[DEBUG] Re-running task %s serially for debug\n", k))
                try({
                  tmp_res <- fit_one_task(batch_task_list[[k]])
                  if (is.null(tmp_res)) {
                    cat(sprintf("[DEBUG re-run] task %s serial re-run returned NULL\n", k))
                  } else {
                    cat(sprintf("[DEBUG re-run] task %s serial re-run returned non-NULL: veg_frac=%.4f barren_frac=%.4f\n", k, as.numeric(tmp_res$vegetated_fraction), as.numeric(tmp_res$barren_fraction)))
                  }
                }, silent = TRUE)
              }
        } else {
          ...
          cat(sprintf("[DEBUG batch_result] training task %s returned non-NULL: veg_frac=%.4f barren_frac=%.4f coef_rows=%d vegs=%s diag_vf=%s\n", k, ca, cb, coef_n, veg_list, as.character(diag_vf)))
        }
      }
    }
  ...
  if (n_keys > 0) {
    ...
    cat("Average time per task: N/A (0 tasks)\n")
  }
  ...
  if (isTRUE(TESTING_MODE)) {
    ...
    # Ensure year and doy columns exist
    if (!"year" %in% names(df_tasks_inference) && "date" %in% names(df_tasks_inference)) {
      ...
      df_tasks_inference$year <- as.integer(lubridate::year(df_tasks_inference$date))
      df_tasks_inference$doy <- lubridate::yday(df_tasks_inference$date)
    }
    
    # Create task_key for inference
    ...
    # Get unique location-year pairs for inference
    # Ensure we use pheno_year consistently so task keys match
    ...
    # Identify target keys for inference
    # Use pheno_year in inference target keys to match df_tasks_inference$task_key
    ...
    if (n_inference_keys > 0) {
      ...
      for (i in seq_along(inference_key_batches)) {
        ...
        # TESTING_MODE: show per-task return status for debugging
        if (isTRUE(TESTING_MODE)) {
          for (k in names(batch_results)) {
            r <- batch_results[[k]]
            if (is.null(r)) {
      ...
      cat(sprintf("Valid inference results: %d\n", length(inference_results_list)))
    } else {
  ...
  if (length(results_list) > 0) {
    ...
    if (barren_one_pct > 50) {
      msg <- sprintf("ERROR: Barren fraction = 1 in %.1f%% of predictions (>%d/%d). This indicates unnormalized endmembers causing bias toward barren. Check endmember normalization in geometric stage 1 unmixing.", barren_one_pct, barren_one_count, length(results_list))
      if (isTRUE(TESTING_MODE)) {
  ...
  if (length(results_list) > 0) {
    ...
    # Ensure pheno_year column exists in all_coefs (year may already represent phenological year)
    if (!"pheno_year" %in% names(all_coefs)) all_coefs$pheno_year <- all_coefs$year
  ...
  aggregate_bootstrap <- function(all_coefs, B = 1000, seed = 123) {
    ...
    for (veg in veg_types) {
      ...
      # Precompute per-row sd (if not available, derive from CI if present), default 0
      if (!"coef_sd" %in% names(veg_data)) {
        veg_data$coef_sd <- NA_real_
      }
      has_ci <- ("coef_025" %in% names(veg_data)) && ("coef_975" %in% names(veg_data))
      veg_data$coef_sd[is.na(veg_data$coef_sd) & has_ci] <- (veg_data$coef_975[is.na(veg_data$coef_sd) & has_ci] - veg_data$coef_025[is.na(veg_data$coef_sd) & has_ci]) / 3.92
      veg_data$coef_sd[!is.finite(veg_data$coef_sd)] <- 0
      ...
      for (b in seq_len(B)) {
        ...
        for (i in seq_along(years)) {
          ...
          if (nrow(yr_data) > 0) {
            # Sample per-location coefficient from uncertainty distribution
            if (any(yr_data$coef_sd > 0, na.rm = TRUE)) {
              sampled <- yr_data$coef
              mask_sd <- is.finite(yr_data$coef_sd) & yr_data$coef_sd > 0
              if (any(mask_sd)) {
                sampled[mask_sd] <- stats::rnorm(sum(mask_sd), mean = yr_data$coef[mask_sd], sd = yr_data$coef_sd[mask_sd])
              }
              sampled <- pmin(1, pmax(0, sampled))
              boot_means[b, i] <- mean(sampled, na.rm = TRUE)
            } else {
  ...
  #' Aggregate PPI-normalized coefficients using bootstrap
  #'
  #' @param all_coefs Data frame with location-level coefficients (location_id, year, Veg, coef)
  #' @param df_tasks Data frame with observation-level data containing PPI and location_id/year
  #' @param B Number of bootstrap resamples
  #' @param seed RNG seed
  #' @return Data frame with PPI-normalized global sums and bootstrap CI
  aggregate_ppi_normalized <- function(all_coefs, df_tasks, B = 500, seed = 123) {
    set.seed(seed)
    if (!requireNamespace("dplyr", quietly = TRUE)) stop("dplyr required")

    # Compute median PPI per location-year
    if (!"PPI" %in% names(df_tasks)) stop("PPI column missing from df_tasks")
    ppi_per_loc_year <- df_tasks %>% dplyr::group_by(location_id, pheno_year) %>% dplyr::summarize(median_ppi = median(PPI, na.rm = TRUE), .groups = "drop")

    merged <- dplyr::left_join(all_coefs, ppi_per_loc_year, by = c("location_id", "pheno_year"))
    if (nrow(merged) == 0) return(data.frame())

    # Filter out rows without PPI
    merged <- merged[!is.na(merged$median_ppi), ]
    veg_types <- unique(merged$Veg[!is.na(merged$Veg)])
    # Exclude barren from PPI-normalized aggregation and plots (PPI-normalization only applies to vegetation)
    veg_types <- veg_types[!tolower(trimws(veg_types)) %in% c("barren")]
    if (length(veg_types) == 0) {
      # Nothing to aggregate after excluding barren
      return(data.frame())
    }
    ...
    for (veg in veg_types) {
      ...
      # Ensure coef_sd present (derive from CI if available)
      if (!"coef_sd" %in% names(veg_data)) veg_data$coef_sd <- NA_real_
      if (all(is.na(veg_data$coef_sd)) && all(c("coef_025", "coef_975") %in% names(veg_data))) {
        veg_data$coef_sd <- (veg_data$coef_975 - veg_data$coef_025) / 3.92
      }
      veg_data$coef_sd[!is.finite(veg_data$coef_sd)] <- 0
      ...
      for (b in seq_len(B)) {
        ...
        for (i in seq_along(years)) {
          ...
          if (nrow(yr_data) > 0) {
            # Sample per-location coefficient according to uncertainty (coef_sd) if available
            if (any(yr_data$coef_sd > 0, na.rm = TRUE)) {
              vals_coef <- yr_data$coef
              mask_sd <- is.finite(yr_data$coef_sd) & yr_data$coef_sd > 0
              if (any(mask_sd)) {
                vals_coef[mask_sd] <- stats::rnorm(sum(mask_sd), mean = yr_data$coef[mask_sd], sd = yr_data$coef_sd[mask_sd])
                vals_coef <- pmin(1, pmax(0, vals_coef))
              }
            } else {
              vals_coef <- yr_data$coef
            }
            vals <- vals_coef * yr_data$median_ppi
            boot_sums[b, i] <- sum(vals, na.rm = TRUE)
          } else {
            boot_sums[b, i] <- NA_real_
          }
        }
      }
      ...
      boot_result <- data.frame(
        ...
        ci_lower = apply(boot_sums, 2, quantile, 0.025, na.rm = TRUE),
        ci_upper = apply(boot_sums, 2, quantile, 0.975, na.rm = TRUE),
        method = "ppi_bootstrap"
      )
  ...
  if (exists("inference_results_list") && length(inference_results_list) > 0) {
    ...
    # Build inference coefficients from inference_results_list
    ...
    if (length(inference_coef_list) > 0) {
      ...
      if (nrow(inference_global_pattern) > 0) {
        ...
        # Compute GLS MBB trend confidence intervals for inference (if function exists)
        if (exists("bootstrap_trend_ci")) {
          cat("Computing GLS MBB trend CI for inference results...\n")
          inference_trend_ci <- tryCatch({
            bootstrap_trend_ci(inference_coefs, B = ifelse(exists("BOOTSTRAP_B"), BOOTSTRAP_B, 1000), block_length = 3)
          }, error = function(e) {
        ...
        # PPI-normalized cumulative for inference (bootstrap aggregated)
        if (exists("inference_coefs") && nrow(inference_coefs) > 0 && "PPI" %in% names(df_tasks_inference) && any(!is.na(df_tasks_inference$PPI))) {
          inference_pattern_ppi <- aggregate_ppi_normalized(inference_coefs, df_tasks_inference, B = ifelse(exists("BOOTSTRAP_B"), BOOTSTRAP_B, 500), seed = 123)
          # Ensure barren is explicitly removed from PPI-normalized results (safety)
          if (nrow(inference_pattern_ppi) > 0) inference_pattern_ppi <- inference_pattern_ppi[!tolower(trimws(inference_pattern_ppi$Veg)) %in% c("barren"), ]
          if (nrow(inference_pattern_ppi) > 0) {
            p_inf_ppi_ts <- ggplot(inference_pattern_ppi, aes(x = year, y = global_coef, color = Veg, group = Veg)) +
              geom_line(size = 1) +
              geom_point(show.legend = FALSE) +
              geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper, fill = Veg), alpha = 0.15, color = NA) +
  ...
  if (nrow(global_pattern_all_barren) > 0) {
    ...
    # Handle case where vegetation data is missing or zero (avoid division by zero or Inf)
  ...
  # Ensure limits are valid (non-singular)
  ...
  # PPI-normalized cumulative per vegetation type (bootstrap aggregated)
  cat("\nGenerating PPI-normalized cumulative plot (bootstrap)...\n")
  if ("PPI" %in% names(df) && any(!is.na(df$PPI))) {
    global_pattern_ppi <- aggregate_ppi_normalized(all_coefs, df, B = ifelse(exists("BOOTSTRAP_B"), BOOTSTRAP_B, 500), seed = 123)
    # Safety: ensure barren is not included in PPI-normalized aggregate
    if (nrow(global_pattern_ppi) > 0) global_pattern_ppi <- global_pattern_ppi[!tolower(trimws(global_pattern_ppi$Veg)) %in% c("barren"), ]
    if (nrow(global_pattern_ppi) > 0) {
